...
Element* Schematic::selectElement(float fX, float fY, bool flag, int *index)
{
  ...
  for(Wire *pw = Wires->last(); pw != 0; pw = Wires->prev()) {
    if(pw->getSelected(x, y)) {
      if(flag) { pw->ElemSelected ^= flag; return pw; }
      if(pe_sel) {
        pe_sel->ElemSelected = false;
        return pw;
      }
      if(pe_1st == 0) pe_1st = pw;  // give access to elements lying beneath
      if(pw->ElemSelected) pe_sel = pw;
    }
    pl = pw->Label;
    if(pl) if(pl->getSelected(x, y)) {
      if(flag) { pl->ElemSelected ^= flag; return pl; }
    ...
    for(pg = pd->Graphs.first(); pg != 0; pg = pd->Graphs.next())
      ...
      for(Marker *pm = pg->Markers.first(); pm != 0; pm = pg->Markers.next())
        if(pm->getSelected(x-pd->cx, y-pd->cy)) {
          if(flag) { pm->ElemSelected ^= flag; return pm; }
          if(pe_sel) {
            pe_sel->ElemSelected = false;
            return pm;
          }
          if(pe_1st == 0) pe_1st = pm; // give access to elements beneath
          if(pm->ElemSelected) pe_sel = pm;
        }
    ...
    if(pd->getSelected(x, y)) {
      if(pd->Name[0] == 'T') {   // tabular, timing diagram or truth table ?
        if(pd->Name[1] == 'i') {
          if(y > pd->cy) {
            ...
            pd->ElemType = isDiagramHScroll;
            return pd;
          }
        }
        else {
          if(x < pd->cx) {      // clicked on scroll bar ?
            pd->ElemType = isDiagramVScroll;
            return pd;
          }
        }
      }
      ...
      for(pg = pd->Graphs.first(); pg != 0; pg = pd->Graphs.next())
        if(pg->getSelected(x-pd->cx, pd->cy-y) >= 0) {
          if(flag) { pg->ElemSelected ^= flag; return pg; }
          if(pe_sel) {
            pe_sel->ElemSelected = false;
            return pg;
          }
          if(pe_1st == 0) pe_1st = pg;  // access to elements lying beneath
          if(pg->ElemSelected) pe_sel = pg;
        }


      if(flag) { pd->ElemSelected ^= flag; return pd; }
      if(pe_sel) {
        pe_sel->ElemSelected = false;
        return pd;
      }
      if(pe_1st == 0) pe_1st = pd;  // give access to elements lying beneath
      if(pd->ElemSelected) pe_sel = pd;
    }
  }
  ...
  for(Painting *pp = Paintings->last(); pp != 0; pp = Paintings->prev()) {
    if(pp->ElemSelected)
      if(pp->resizeTouched(fX, fY, Corr))
        if(pe_1st == 0) {
          pp->ElemType = isPaintingResize;
          return pp;
        }

    if(pp->getSelected(fX, fY, Corr)) {
      if(flag) { pp->ElemSelected ^= flag; return pp; }
      if(pe_sel) {
        pe_sel->ElemSelected = false;
        return pp;
      }
      if(pe_1st == 0) pe_1st = pp;  // give access to elements lying beneath
      if(pp->ElemSelected) pe_sel = pp;
    }
  }
